...
public class DeviceRegistrationOptionalAmqpIT {
    ...
    public void testAssertRegistrationSucceedsForGateway(final TestContext ctx) {
        ...
        final Future fut1 = helper.registry.registerDevice(Constants.DEFAULT_TENANT, deviceId,
                new JsonObject().put(RegistrationConstants.FIELD_ENABLED, true)
                        .put("via", gatewayId));
        final Future fut2 = helper.registry.registerDevice(Constants.DEFAULT_TENANT, gatewayId,
                new JsonObject().put(RegistrationConstants.FIELD_ENABLED, true));

        CompositeFuture.all(fut1, fut2).setHandler(r -> {
            // verify
    ...
    public void testAssertRegistrationFailsForUnauthorizedGateway(final TestContext ctx) {
        ...
        final Future fut2 = helper.registry.registerDevice(Constants.DEFAULT_TENANT, gatewayId,
                new JsonObject().put(RegistrationConstants.FIELD_ENABLED, true));
        final Future fut3 = helper.registry.registerDevice(Constants.DEFAULT_TENANT, gatewayId2,
                new JsonObject().put(RegistrationConstants.FIELD_ENABLED, true));

        CompositeFuture.all(fut1, fut2, fut3).setHandler(r -> {
            // verify
            deviceRegistryclient.getOrCreateRegistrationClient(Constants.DEFAULT_TENANT)
